...
def create_data(channels =3):
    ...
    data_len =20000
    val_change =[0.02,-0.02,0.01,0.1]
    signs = [0.5 ,-0.5,-0.3,-0.4]
    val_change =[0.2,0.2]#,0.01,0.1]
    signs = [0.5 ,-0.5,]#-0.3,-0.4]
    total_set = []
    j=0
...
#             new_val =new_val+(np.sin(i*3.14*val)*np.cos(i*0.28*val)) -signs[j] *np.cos(i*0.03)*np.sin(i*0.07)+0.015*np.cos(0.55*i)*np.sin(0.51*i)-rand
            new_val = signs[j]* np.sin(i*val_change[j]/10)*np.cos(i*val_change[j]) + 2 
            if new_val < 0.001:
                new_val = 0.001
        ...
        # plt.show()
    print('Done creating test data', total_set.shape)
    return total_set
