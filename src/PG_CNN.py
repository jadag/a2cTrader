...
class CNNPolicyGrad(PolicyGrad):
    ...
    def model(self, x, pos,training):
        ...
        with tf.name_scope('fullyconnected_'+self.name) as scope:
            fc  _cnn = tf.layers.dense(inputs=flat_tensor,
                                       units=30,
    ...
    def get_action(self, observation):
        ...
        action_entry =[0]*self.output_sz
        max_action= np.argmax(action)
        action_entry[max_action] =1 
        self.actions.append(max_action)

        return np.array(action_entry)

    def loss_function(self, logits):
        log_prob = tf.log(tf.nn.softmax(logits))
        entries = tf.range(tf.shape(log_prob)[0])*tf.shape(log_prob)[1]
        log_vector = tf.reshape(log_prob,[-1])
        indices = entries + self.Y
        cost = tf.gather(log_vector, indices)
        
        loss = -tf.reduce_mean(tf.multiply(cost, self.discounted_rewards))
...
#         loss = -tf.reduce_sum(logits,axis=1)*self.discounted_rewards #+ regularization*self.alpha
        self.loss_func = loss
...
#             regularization = tf.Print(regularization,[regularization],'sdf')
    ...
    def insert_data(self, actions_, positions_, quotes_, rewards_):
        self.actions.append(actions_)
        self.positions.append(positions_)
        self.quotes.append(quotes_)
...
#         actions = actions.reshape(data_len, self.output_sz)
        ...
        for batch_end in range(batch_sz, total_sz, batch_sz):
            ...
            _,c =self.tf_sess.run([self.optimiz,self.loss_func], 
                                  feed_dict ={self.X: batch_quote, self.pos:batch_position, 
                                   self.Y:batch_actions, self.discounted_rewards:batch_disc_rewards,
                                   self.training :True})
            print('cost ', c)
